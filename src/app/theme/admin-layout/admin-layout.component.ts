...
export class AdminLayoutComponent implements OnInit, OnDestroy {
  ...
  getSearchResults(term) {
    this.search_results = term;
  }

  getTotalResults(term) {
    this.totalResults = term;
  }

  getSearchVisible(term) {
    this.search_visible = term;
  }
  ...
  isString(term): boolean {
    return typeof term === 'string';
  }

  highlightText(term): string {
    let re = new RegExp(this.search_term,"g");
    let highlightedText = term.value.replace(re, "<mark>$&</mark>");
    return term.key + ': ' + highlightedText;
  }
